...
t_string	ft_format_size_e(long double nb, char *flags)
{
	...
	nb_exposant = 0;
	if (nb > 0.0)
		while((nb /= 10.0) > 10.0 && nb_exposant <= 110 )
			nb_exposant++;
	else if (nb < 0.0)
		while((nb *= 10.0) < 0.0 && nb_exposant <= 110)
			nb_exposant++;
...
char	*ft_format_e(va_list ap, char *flags, int i)
{
	...
	size = ret.size - 1;
	pow = ft_get_pow(v);
	v = pow == 0 ? v : v * (10.0 * (long double)pow);
	write(1, "coucou\n", 7);
	while ((pow /= 10) > 9)
	...
	v = prec > 0.0 ? v * (10.0 * prec) : v;	
	while ((v /= 10.0) > 9.0)
		ret.str[--size] = ft_dmod(v, 10.0) + '0';
	if (prec > 0)
		ret.str[--size] = '.';
	ret.str[--size] = ft_dmod(v, 10.0) + '0';
	return (ret.str);
